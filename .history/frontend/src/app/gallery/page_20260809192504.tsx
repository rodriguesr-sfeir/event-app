  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        {/* Export Button */}
        {!loading && photos.length > 0 && isAdmin && (
          <div className="mb-8">
            <button
      ...
      {lightboxOpen && photos.length > 0 && (
        ...
        >
          ...
          >
            <img
              src={`https://ldtxknyhnlijxewqxcpy.supabase.co/storage/v1/object/public/event-photos/${photos[lightboxIndex].processed_image_path}`}
              alt="Full view"
            ...
            <div className="mt-4 text-center">
              ...
              {photos[lightboxIndex].guest_name && (
                <p className="text-gray-300 text-sm mt-2">
                  par {photos[lightboxIndex].guest_name}
                </p>
              )}
              {photos[lightboxIndex].guest_description && (
                <p className="text-gray-300 text-sm mt-2">
                  {photos[lightboxIndex].guest_description}
                </p>
              )}
              ...
              <div className="flex gap-4 justify-center mt-4">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    toggleLike(photos[lightboxIndex].id);
                  }}
                  className="bg-white/20 hover:bg-white/30 text-white p-3 rounded-full transition transform hover:scale-110"
                >
                  {likedPhotos.has(photos[lightboxIndex].id) ? '❤️' : '🤍'}
                </button>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    copyPhotoLink(photos[lightboxIndex].id);
                  }}
                  className="bg-white/20 hover:bg-white/30 text-white p-3 rounded-full transition transform hover:scale-110"
                >
                  🔗
                ...
                {isAdmin && (
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      downloadPhoto(
                        photos[lightboxIndex].processed_image_path,
